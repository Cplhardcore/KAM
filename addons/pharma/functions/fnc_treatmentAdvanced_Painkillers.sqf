...
if (_painkillerscount > 0 && GVAR(painkillerStack) && _painSuppress < 0.2 && _painSuppress > 0.11) exitWith {[_patient, "Painkillers", 90, 600, 10, 0.1, -5] call ace_medical_status_fnc_addMedicationAdjustment};
if (_painkillerscount > 0 && GVAR(painkillerStack) && _painSuppress < 0.1 && _painSuppress > 0) exitWith {[_patient, "Painkillers", 90, 600, 10, 0.2, -5] call ace_medical_status_fnc_addMedicationAdjustment};
