...
if (GVAR(kidneyAction)) then {
    [{
        ...
        private _microDrive =
        (
            linearConversion [1.2,2.4,_shockIndex,0,0.015,true] +
            linearConversion [2,10,_lactate,0,0.02,true] +
            linearConversion [7.35,7.0,_ph,0,0.02,true]
        );
        _micro = _micro + (_microDrive * 5);
        if (_shockIndex < 1.1 && _lactate < 2.5 && _ph > 7.32) then {
            _micro = _micro - 0.01;
        };
        _micro = (_micro max 0) min 1;
        _unit setVariable [QGVAR(microcirculation),_micro,true];
        private _mito = _unit getVariable [QGVAR(mitoFailure),0];
        private _mitoDrive =
        linearConversion [6,12,_lactate,0,0.02,true] +
        linearConversion [7.2,6.8,_ph,0,0.02,true];

        _mito = _mito + (_mitoDrive * 5);

        if (_lactate < 3 && _ph > 7.32) then {
            _mito = _mito - 0.01;
        };

        _mito = (_mito max 0) min 1;

        _unit setVariable [QGVAR(mitoFailure),_mito,true];
        ...
        if (_micro > 0.85 && _lactate > 8) then {    
