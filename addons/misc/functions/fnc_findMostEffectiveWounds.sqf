...

{
    ...
    if (isClass (_config >> _className)) then {
        ...
        if (isNumber (_woundTreatmentConfig >> "allowRollover")) then {
...
if (ACEGVAR(medical_treatment,bandageRollover) && {_bandageRemaining > 0} && (_allowRollover == 1)) then {
    [_patient, _bandage, _bodyPart, _bandageRemaining, _foundWounds] call ACEFUNC(medical_treatment,findMostEffectiveWounds);
};
