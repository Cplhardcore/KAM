...
private _hasPneumothorax = (_patient getVariable [QGVAR(pneumothorax), [0, 0]] select 0 > 2 || _patient getVariable [QGVAR(pneumothorax), [0, 0]] select 1 > 2 || (_tension select 0) || (_tension select 1) || ((_hemo select 0) > 0) || ((_hemo select 1) > 0));
...
if (GET_BREATHING_RATE(_patient) isEqualTo 0) then {
    ...
    if (_hasPneumothorax && _airwayClear) then {
        ...
        if (_simpleSetting) then {
            ...
            if (((_patient getVariable [QGVAR(hemopneumothorax), [0, 0]] select 0) > 0.2) || ((_patient getVariable [QGVAR(hemopneumothorax), [0, 0]] select 1) > 0.2)) then {
                _hintSize = 2.5;
                _type = LLSTRING(Hemopneumothorax);
                _type_log = LLSTRING(Hemopneumothorax_short);
            };
