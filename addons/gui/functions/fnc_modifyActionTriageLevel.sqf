...
if (
    ACEGVAR(medical_gui,interactionMenuShowTriage) == 1 // Anyone
    || {ACEGVAR(medical_gui,interactionMenuShowTriage) == 2 && {[_player] call ACEFUNC(medical_treatment,isMedic)}} // Medics & Doctors
) then {
    private _colorHex = switch (_target getVariable [QACEGVAR(medical,triageLevel), 0]) do {
        case 1: {
        ...
        case 4: {
            [TRIAGE_COLOR_EXPECTANT] call BIS_fnc_colorRGBtoHTML
        };
        case 5: {
